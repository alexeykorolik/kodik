...
export function CodePreview({ code, highlights = [] }: { code: string; highlights?: number[] }) {
  return <pre className="python-code"><code>{code.split('\n').map((line, index) => <span className={`code-line ${highlights.includes(index) ? 'code-highlight' : ''}`} key={index}><span className="line-number" aria-hidden="true">{index + 1}</span><span>{line.split(/("(?:\\.|[^"\\])*"|\b(?:print|if|else|for|in|range|def|pass|True|False)\b|\b\d+\b)/g).map((token, i) => <span key={i} className={token.startsWith('"') ? 'syntax-string' : /^(print|if|else|for|in|range|def|pass|True|False)$/.test(token) ? 'syntax-keyword' : /^\d+$/.test(token) ? 'syntax-number' : ''}>{token}</span>)}</span></span>)}</code></pre>
}
