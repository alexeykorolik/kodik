...
export type Progress = LearningProgress & { completed: number[]; currentLesson?: number; started?: boolean; drafts?: Record<string, object> }
...
export function cleanProgress(value: unknown): Progress {
  ...
  if (p.version !== 2) for (const id of [1,2,6,7]) delete drafts[id]
  return { completed, version: 2, bestStars, sessions, introducedConcepts,
