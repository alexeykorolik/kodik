...
test.describe('новые механики пальцем',()=>{
  ...
  for(const id of [6,2,3,9,5,10,4,11]) test(`знакомство ${id}: ${lessons.find(l=>l.id===id)!.title}`,async({page})=>{
    ...
    if(id===2){ await createVariable(page,'имя'); await add(page,'Присвоить'); await add(page,'Текст'); await text(page,'Мира'); await add(page,'Напечатать'); await add(page,'Получить переменную') }
...
test('звёзды не блокируют освоенные навыки',async({page})=>{
  ...
  await page.getByRole('button',{name:'Нужна подсказка?'}).click();await page.getByRole('button',{name:'Ещё подсказка'}).click()
  await passed(page);await expect(page.locator('.feedback-panel').getByLabel('1 из 3 звёзд')).toBeVisible()
...
for(const id of [15,16,19,20,21]) test(`переход к коду ${id}`,async({page})=>{
  ...
  await passed(page);await expect(page.locator('.feedback-panel').getByLabel('3 из 3 звёзд')).toBeVisible()
})
...
test('повторное открытие примера не расходует подсказку после перезагрузки',async({page})=>{
  ...
  await passed(page);await expect(page.locator('.feedback-panel').getByLabel('2 из 3 звёзд')).toBeVisible()
})

test('работа без сети, очистка, отмена и системные ошибки хранения',async({page,context})=>{
  await seed(page,13)
  await add(page,'Напечатать');await add(page,'Текст');await text(page,'Мне нравится Python')
  await page.getByLabel('Действия с программой').click();await page.getByRole('button',{name:'Очистить программу',exact:true}).click();await page.getByRole('dialog').getByRole('button',{name:'Очистить',exact:true}).click()
...
test('блок и строка Python синхронизируют фокус в обе стороны',async({page})=>{
  ...
  expect(await page.locator('.live-mirror').getAttribute('data-selected-id')).toBe(await line.getAttribute('data-source-id'))
  await expect(line).toHaveClass(/code-highlight/)
  await line.click()
...
test('все 22 задания подряд, звёзды открывают главы, финал',async({page})=>{
  ...
  const create=async(name:string)=>createVariable(page,name)
  ...
  for(let i=0;i<lessons.length;i++){
    ...
    switch(l.id){
      ...
      case 8: await page.getByLabel('Другие действия').click();await page.getByRole('button',{name:'Показать все блоки'}).click();await page.locator('.text_print > .math_arithmetic > .blocklyDropdownField').click();await page.getByRole('option',{name:/×/}).click();break
    ...
    await passed(page)
    if(!l.tutorial) await expect(page.locator('.feedback-panel').getByLabel('3 из 3 звёзд')).toBeVisible()
    await page.getByRole('button',{name:'Продолжить',exact:true}).click()
